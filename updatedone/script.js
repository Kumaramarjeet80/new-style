        ...
        .then(res => {
          if (res && res.valid === false) {
            showMohnaPopup({
              ...
              message: 'Your account was just accessed on another device. For security reasons, you have been logged out.',
...
function verifyAndCompleteSignup() {
  ...
  fetch(SPREADSHEET_API_URL, {
    ...
    body: JSON.stringify({
      ...
      name: name, phone: phone, email: email, password: password, username: username, otp: otp,
...
function renderCheckoutCart() {
  ...
  var customWalletVal = parseFloat(document.getElementById('customWalletInput')?.value) || 0;
  var walletDeduction = 0;
  if (userWalletBalance > 0 && customWalletVal > 0) {
    walletDeduction = Math.min(netBeforeWallet, Math.min(customWalletVal, userWalletBalance));
  }
...
function saveOrderToBackend(paymentId, finalAmount, subtotal, promoDiscount, maxDeliveryFee, walletDeduction) {
  ...
  var appliedCoupon = document.getElementById('couponCodeInput')?.value.trim().toUpperCase() || "";
  ...
  .then((res) => {
    ...
    var wInput = document.getElementById('customWalletInput');
...
function renderProfilePage() {
  ...
  var usr = document.getElementById('profUsername'); if(usr) usr.value = currentUser.username ? `@${currentUser.username}` : '@user';
