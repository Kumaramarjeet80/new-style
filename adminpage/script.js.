var SPREADSHEET_API_URL = "https://script.google.com/macros/s/AKfycbzU3HoF5MLrOGXtRJPW-b4uZ3t97RPwnA1-QuzgwSTtZPXIFLxyym9tFNEmLP93tvvDHw/exec";
...
function manualRefreshRiderTracking() {
  fetchInitialStoreData(true);
  setTimeout(() => {
    showMohnaPopup({ type: 'success', title: 'Radar Refreshed', message: 'Latest rider telemetry synchronized successfully!', primaryText: 'OK' });
  }, 400);
}

function openZoomedQrModal() {
  if (!activeSelectedModalOrder || !activeSelectedModalOrder.deliveryToken) return;
  var zoomModal = document.getElementById('zoomedQrModal');
  var zoomTarget = document.getElementById('zoomedQrTarget');
  if (zoomTarget) zoomTarget.innerHTML = "";
  if (zoomModal && zoomTarget) {
    zoomModal.style.display = 'flex';
    new QRCode(zoomTarget, {
      text: JSON.stringify({
        type: "MOHNA_DELIVERY_HANDOVER",
        orderId: activeSelectedModalOrder.orderId,
        deliveryToken: activeSelectedModalOrder.deliveryToken
      }),
      width: 220,
      height: 220
    });
